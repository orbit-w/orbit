...
// UpdateValue updates the value of the item with the given key.
// It returns true if the item exists and the value was updated, otherwise it returns false.
//
// UpdateValue 更新具有给定键的项的值。
// 如果该项存在并且值已更新，则返回 true，否则返回 false。
func (pq *PriorityQueue[K, V, S]) UpdateValue(k K, v V) bool {
	if item, exist := pq.items[k]; exist {
		item.Value.Value = v
		return true
	}
