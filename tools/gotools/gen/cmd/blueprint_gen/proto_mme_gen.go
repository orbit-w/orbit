...
func (g *ProtoGenerator) generateMechanismsProto(outputDir string) error {
	...
	for _, mech := range g.data.Mechanisms {
		// 添加注释（使用 Mechanism 名称作为注释）
		if mech.Name == "LevelUpMechanism" {
			sb.WriteString("// 升级机制\n")
		} else if mech.Name == "HeroMechanism" {
			sb.WriteString("// 英雄机制\n")
		} else if mech.Name == "ManualUnlockMechanism" {
			sb.WriteString("// 手动解锁机制\n")
		} else if mech.Name == "WearMechanism" {
			sb.WriteString("// 穿戴机制\n")
		} else {
			sb.WriteString(fmt.Sprintf("// %s\n", mech.Name))
		}
