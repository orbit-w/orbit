...
type Subscriber struct {
	...
	Entities       map[int64]mme.EntityType              // 已订阅的 Entities ID 列表, 用于快速遍历, 保证订阅顺序
	EntitiesByType map[mme.EntityType]map[int64]struct{} // 已订阅的 Entities 类型和 Entity 列表, Key 为 Entity 类型, Value 为 Entity 列表
}
...
func NewSubscriber(uuid string, strategy ISubscribeStrategy) *Subscriber {
	return &Subscriber{
		UUID:           uuid,
		Strategy:       strategy,
		Entities:       make(map[int64]mme.EntityType),
		EntitiesByType: make(map[mme.EntityType]map[int64]struct{}),
	}
}
...
func (s *Subscriber) ClearSubscribedEntities() {
	s.Entities = make(map[int64]mme.EntityType)
	s.EntitiesByType = make(map[mme.EntityType]map[int64]struct{})
}

// 订阅 Entity
func (s *Subscriber) SubscribeEntity(entity mme_agent.IEntity) {
...
func (s *Subscriber) unsubscribeEntityById(entityId int64, entityType mme.EntityType, clear bool) {
